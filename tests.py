...
from pptracer import make_logp, posterior_inference
...
def gg_sampler(rs):
  z = rs.normal(np.zeros(D), np.ones(D))
  x = rs.normal(np.tile(z, (N, 1)), np.ones((N, D)))
  return x, z
...
def test_posterior_samples():
  ...
  x = np.tile(rs.normal(np.ones((N, 1)), np.ones((N, 1))), (1, D))
  observations = (x, None)
  samples = posterior_inference(gg_sampler, observations,
                                0.1, 10, 50)
  z, = samples[-1]
